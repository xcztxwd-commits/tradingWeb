...
import com.fxplatform.trading.scenario.oracle.SpotScenarioOracle;
...
class ScenarioResilienceIT {
  ...
  private final SpotScenarioOracle spotOracle = new SpotScenarioOracle();
  private final PerpetualScenarioOracle perpetualOracle = new PerpetualScenarioOracle();
  ...
  void executesScenario(String caseId, ScenarioDefinition scenario) {
    ...
    List<ExpectedScenarioResult> expectations = calculateAll(scenario);
  ...
  private List<ExpectedScenarioResult> calculateAll(ScenarioDefinition scenario) {
    return scenario.productType() == ProductType.CRYPTO_SPOT
        ? spotOracle.calculateAll(scenario)
        : perpetualOracle.calculateAll(scenario);
  }
