...
describe('orders, positions and wallet platform routes', () => {
  ...
  it('keeps dialogs and controlled edit state in the shared route model across resize', () => {
    ...
    const ordersContent = source('shared-widgets', 'orders', 'OrdersRouteContent.tsx')
    const positionsContent = source('shared-widgets', 'positions', 'PositionsRouteContent.tsx')
    assert.match(ordersTypes, /editingOrder/)
    assert.match(ordersTypes, /modifyFields/)
    assert.match(ordersTypes, /pendingCancelOrder/)
    assert.match(ordersContent, /<Dialog/)
    assert.match(ordersContent, /priority="critical"/)
    assert.match(positionsTypes, /pendingClosePosition/)
    assert.match(positionsTypes, /protectionForm/)
    assert.match(positionsContent, /import \{ Dialog,/)
    assert.match(positionsContent, /<Dialog[\s\S]*priority="critical"/)
    assert.doesNotMatch(positionsContent, /role="dialog"/)
