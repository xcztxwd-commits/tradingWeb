...
describe('mobile trading terminal redesign', () => {
  it('adds a mobile-only terminal shell instead of compressing the desktop workspace', () => {
    ...
    assert.equal(existsSync(join(tradingDir, 'useMobileTerminalViewport.ts')), false)
    assert.match(deviceClassSource, /mobileViewportQuery = '\(max-width: 900px\)'/)
    assert.match(tradingPageSource, /import \{ useDeviceClass \} from '\.\.\/\.\.\/app\/device\/DeviceClassProvider'/)
    assert.match(tradingPageSource, /const deviceClass = useDeviceClass\(\)/)
    assert.match(tradingPageSource, /\{deviceClass === 'pc' \? <TradingDesktopView \{\.\.\.viewProps\} \/> : null\}/)
    assert.match(tradingPageSource, /\{deviceClass === 'mobile' \? <TradingMobileView \{\.\.\.viewProps\} \/> : null\}/)
